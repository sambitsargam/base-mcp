...
export async function listNftsHandler(
  ...
  try {
    ...
    console.log(`Listing NFTs for address: ${args.ownerAddress}`);
...
export async function transferNftHandler(
  ...
  try {
    ...
    console.log(`Transferring NFT ${args.tokenId} from contract ${args.contractAddress} to ${args.toAddress}`);
    ...
    const txUrl = constructBaseScanUrl(wallet.chain ?? base, txHash as `0x${string}`);
